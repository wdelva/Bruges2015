...
readthedata <- function(DestDir, outputID){
  ...
  outputtables <- return(list(ptable = ptable, rtable = rtable, etable = etable))
}
...
phylo4(nj(geodist(g2)$gdist))
  ...
  x <- # first row must be root node
    
  phylo4(x, )
 
}
head(datalist$etable)
